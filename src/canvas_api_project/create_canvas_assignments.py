...
def internalize_structure(
    data: Dict[str, List[RawAssignmentGroupData]],
) -> List[InternalizedAssignmentGroupData]:
    ...
    for i, group in enumerate(data.get("assignment_groups", []), start=1):
        ...
        internalized_group: InternalizedAssignmentGroupData = {
            "name": group_name,
            "questions": [],
        }
        ...
        for j, question in enumerate(group.get("questions", []), start=1):
            ...
            question_due_date_str: Optional[str] = question.get(
                "due_date", default_due_date_str
            )

            if not question_due_date_str:
                raise ValueError(
                    f"No due date specified for '{question_name}' in '{group_name}'."
                )
...
def print_structure(internalized_data: List[InternalizedAssignmentGroupData]) -> None:
    ...
    for group in internalized_data:
        ...
        for question in group["questions"]:
            print(
                f"  - {question['name']} (Points: {question['points']}, Due: {question['due_date']})"
            )
...
def check_existing_groups(
    canvas_course, internalized_data: List[InternalizedAssignmentGroupData]
) -> bool:
    existing_groups = {group.name for group in canvas_course.get_assignment_groups()}
    for group in internalized_data:
        if group["name"] in existing_groups:
            raise ValueError(
                f"Error: Assignment group '{group['name']}' already exists."
            )
    return True
...
def create_assignments(
    ...
    for group in internalized_data:
        ...
        for question in group["questions"]:
            ...
            print(
                f"  Created assignment '{question_name}' with {points} points and due date {due_date}."
            )
...
def main(course_id: int, file_path: str) -> None:
    ...
    try:
        internalized_data: List[InternalizedAssignmentGroupData] = (
            internalize_structure(raw_data)
        )
    except ValueError as e:
    ...
    if confirm_input(
        "Proceed with creating the assignments? (yes/no): ", handle_no_response
    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create Canvas assignments from a YAML file."
    )
    parser.add_argument("course_id", type=int, help="The ID of the Canvas course.")
    parser.add_argument(
        "file_path",
        type=str,
        help="The path to the YAML file containing assignment data.",
    )
